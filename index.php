...
<body class="bg-slate-50 text-slate-900 min-h-screen flex flex-col">
            ...
            <div class="bg-white p-10 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div class="flex flex-wrap gap-4 pt-6 border-t border-slate-100">
                    ...
                    <?php else: ?>
                        ...
                        <?php endif; ?>
                        <a href="pages/suivi_etudiants.php" class="bg-white text-slate-900 font-bold text-sm px-6 py-3 rounded-lg hover:bg-slate-50 border border-slate-300">Voir mes élèves (Tuteur)</a>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <?php else: foreach($offres_preview as $o): ?>
                            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col relative group">
                                ...
                                <div class="p-6 pt-10 flex-grow flex flex-col justify-between">
                                    ...
                                    <a href="pages/offres.php" class="block w-full text-center bg-slate-50 border border-slate-200 text-slate-700 font-bold py-2.5 rounded-lg text-sm hover:bg-slate-100 transition-colors">Consulter l'offre</a>
                                </div>
                            </div>
    ...
    <!-- MODAL REGLEMENT REPARÉE -->
    <div id="modal-reglement" style="display: none;" class="fixed inset-0 bg-slate-900/80 z-50 items-center justify-center p-4">
        <div class="bg-white rounded-xl p-10 w-full max-w-2xl max-h-[80vh] overflow-y-auto shadow-2xl relative">
            <button type="button" onclick="document.getElementById('modal-reglement').style.display='none'" class="absolute top-6 right-6 text-slate-400 hover:text-rose-500 font-black text-2xl">&times;</button>
            <h2 class="text-xl font-bold mb-4 text-slate-900">Règlement</h2>
            <p class="text-sm text-slate-600 leading-relaxed mb-6">Le stage doit se dérouler sur une période minimale de 10 semaines. L'étudiant s'engage à respecter le règlement intérieur de l'entreprise d'accueil. Toute absence doit être signalée à l'IUT et à l'entreprise.</p>
            <button type="button" onclick="document.getElementById('modal-reglement').style.display='none'" class="bg-slate-900 text-white font-bold px-6 py-2 rounded hover:bg-slate-800 transition-colors">Fermer</button>
        </div>
    </div>
