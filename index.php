...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col">
    <nav class="bg-white border-b border-slate-200 py-4 px-8 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <ul class="flex items-center gap-6 text-sm font-medium">
            ...
            <?php elseif ($role === 'Administrateur'): ?>
                <li><a href="pages/validation_comptes.php" class="text-slate-600 hover:text-blue-600">Validation & Permissions</a></li>
            <?php elseif (in_array($role, ['Enseignant', 'Responsable de stage', 'Chef de departement'])): ?>
                <li><a href="#" class="text-slate-600 hover:text-blue-600">Suivi Étudiants (<?= $role ?>)</a></li>
            <?php endif; ?>
    ...
    <main class="flex-grow flex items-center justify-center p-8">
        <div class="max-w-2xl bg-white p-12 rounded-2xl shadow-sm border border-slate-100 text-center">
            ...
            <?php else: ?>
                <a href="pages/profil.php" class="inline-block bg-emerald-600 text-white font-bold text-xs uppercase px-6 py-3 rounded-xl shadow-md hover:bg-emerald-500">Faire une demande d'évolution</a>
            <?php endif; ?>
        </div>
    </main>
</body>
